-- Bronze layer: raw FRED API payload retention.
-- Parameterize {{catalog}}. Column names mirror fred_pipeline.bronze.

USE CATALOG {{catalog}};

CREATE TABLE IF NOT EXISTS bronze.fred_api_response (
    run_id            STRING NOT NULL,
    series_id         STRING NOT NULL,
    endpoint          STRING,
    request_params    STRING,   -- JSON string of request params (api_key redacted)
    response_payload  STRING,   -- verbatim FRED JSON response
    observation_count INT,
    payload_bytes     INT,
    ingested_at       TIMESTAMP
)
USING DELTA
PARTITIONED BY (series_id)
COMMENT 'Raw, verbatim FRED API responses (system of record)'
TBLPROPERTIES (
    delta.autoOptimize.optimizeWrite = true,
    delta.autoOptimize.autoCompact = true
);
