...
``fred_pipeline.manifest`` moved to :mod:`fred_pipeline.catalogs.manifest` as part of
...
so every attribute access, ``from fred_pipeline.manifest import X``, and any
test monkeypatching by the old string path all operate on the same
underlying module. New code should still import from
:mod:`fred_pipeline.catalogs.manifest` directly.
"""

import sys as _sys

from fred_pipeline.catalogs import manifest as _real_module
