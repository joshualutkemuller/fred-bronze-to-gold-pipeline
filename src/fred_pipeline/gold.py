...
``fred_pipeline.gold`` moved to :mod:`fred_pipeline.writer.gold` as part of
...
so every attribute access, ``from fred_pipeline.gold import X``, and any
test monkeypatching by the old string path all operate on the same
underlying module. New code should still import from
:mod:`fred_pipeline.writer.gold` directly.
"""

import sys as _sys

from fred_pipeline.writer import gold as _real_module
