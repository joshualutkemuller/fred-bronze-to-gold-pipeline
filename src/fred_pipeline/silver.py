...
``fred_pipeline.silver`` moved to :mod:`fred_pipeline.data.silver` as part of
...
so every attribute access, ``from fred_pipeline.silver import X``, and any
test monkeypatching by the old string path all operate on the same
underlying module. New code should still import from
:mod:`fred_pipeline.data.silver` directly.
"""

import sys as _sys

from fred_pipeline.data import silver as _real_module
