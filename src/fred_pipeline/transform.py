...
``fred_pipeline.transform`` moved to :mod:`fred_pipeline.data.transform` as part of
...
so every attribute access, ``from fred_pipeline.transform import X``, and any
test monkeypatching by the old string path all operate on the same
underlying module. New code should still import from
:mod:`fred_pipeline.data.transform` directly.
"""

import sys as _sys

from fred_pipeline.data import transform as _real_module
