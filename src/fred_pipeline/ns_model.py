...
``fred_pipeline.ns_model`` moved to :mod:`fred_pipeline.ml.ns_model` as part of
...
so every attribute access, ``from fred_pipeline.ns_model import X``, and any
test monkeypatching by the old string path all operate on the same
underlying module. New code should still import from
:mod:`fred_pipeline.ml.ns_model` directly.
"""

import sys as _sys

from fred_pipeline.ml import ns_model as _real_module
